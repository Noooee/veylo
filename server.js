...
async function initDatabase() {
  ...
  // ★ 重要 ★
  //
  // 既存のroomsテーブルにowner_idが存在しない
  // 場合でも動くようにする。
  //
  // 以前のDBに「owner」という列があるため、
  // owner_idを追加してownerの値を移行する。
  // ==================================================
  ...
  // ==================================================
  // owner列が存在する場合、
  // owner_idがNULLのレコードへコピーする。
  //
  // PostgreSQLでは存在しない列を直接参照すると
  // エラーになるので、information_schemaで確認。
  // ==================================================

  const ownerColumnCheck = await pool.query(`
    SELECT 1
  ...
  if (ownerColumnCheck.rows.length > 0) {
    console.log(
      "既存の rooms.owner を rooms.owner_id へ移行しています..."
    );

    await pool.query(`
      UPDATE rooms
      SET owner_id = CAST(owner AS INTEGER)
      WHERE owner_id IS NULL
        AND owner IS NOT NULL
        AND CAST(owner AS TEXT) ~ '^[0-9]+$'
    `);
  }

  // owner_idに外部キーが付いていない既存DBにも対応
  const ownerFkCheck = await pool.query(`
    SELECT
      tc.constraint_name
  ...
  if (ownerFkCheck.rows.length === 0) {
    try {
      ...
      // すでに別名の外部キーが存在する場合などは無視
      console.log(
        "rooms.owner_id の外部キー追加をスキップ:",
        error.message
      );
    }
  }
